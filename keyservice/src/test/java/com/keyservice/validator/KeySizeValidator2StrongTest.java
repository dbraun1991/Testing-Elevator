...
/**
 * P1 — Live-Fix (starke Tests)
 *
 * Alle erlaubten Werte, alle relevanten ungültigen Werte,
 * Boundaries direkt neben den erlaubten Werten.
 *
...
class KeySizeValidator2StrongTest {
    ...
    void allowedSizesAreValid(int size) {
        assertThat(validator.isValid(size)).isTrue();
    }
    ...
    void invalidSizesAreRejected(int size) {
        assertThat(validator.isValid(size)).isFalse();
    }
    ...
    void arbitraryLargeValue_isInvalid() {
        assertThat(validator.isValid(99999)).isFalse();
    }
}